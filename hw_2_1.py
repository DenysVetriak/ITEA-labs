...
def type_first_number():
    ...
    while ATTEMPTS:
        ...
        # Контролировать ввод пользователя еще не научились так как не работали с исключениями.
        if number_one >= 0:
            return number_one
        else:
            ValueError(print("Number, {}, is not natural number".format(number_one)))
    else:
...
def type_second_number():
    ...
    while ATTEMPTS:
        ...
        if number_two >= 0:
            ...
            ValueError(print("Number, {}, is not natural number".format(number_two)))
    else:
        exit("Your attempts are finished, try to reload program")
...
def sum_natural():
    ...
    while ATTEMPTS:  # Убедимся что числа введены правильно и первое число меньше второго.
        ...
        if x1 <= x2:
            ...
            print("Number {} more than {}, wrong format".format(x1, x2))
    else:
